...
from virus_model.policies import (
    BaselineInfectionPolicy,
    ExtensionInfectionPolicy,
    InfectionPolicy,
)
from virus_model.stats import TickRecord, record_tick
from virus_model.world import World


def _default_policy(config: SimulationConfig) -> InfectionPolicy:
    if config.immune_reinfection_probability > 0:
        return ExtensionInfectionPolicy(config)
    return BaselineInfectionPolicy(config)
...
class VirusSimulation:
    def __init__(
        ...
        *,
        ...
        self.policy = policy or _default_policy(config)
